          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {isInstallable && (
                <button
                  onClick={install}
                  className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-muted hover:bg-muted/80"
                >
                  <Download size={16} />
              ...
              {/* iOS Install Button (instructional) */}
              {isIOS() && !isInStandaloneMode() && (
                <button
                  onClick={() => setShowIOSPopup(true)}
                  className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm bg-muted"
                >
                  <Share size={16} />
                  {t('تثبيت على الآيفون', 'Installer sur iPhone')}
                </button>
              )}

              {/* Auth */}
              {isAuthenticated ? (
                <Link to="/dashboard" className="hidden sm:inline-flex btn-primary gap-2">
                  <User size={16} />
                  {t('التطبيق', 'Mon App')}
                </Link>
              ) : (
